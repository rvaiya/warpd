...
static int draw_text(Drawable win, int x, int y, XftColor *color, const char *s)
{
	...
	if (win != _win) {
		free(drw);
		drw = XftDrawCreate(dpy, win, DefaultVisual(dpy, scr),
				    DefaultColormap(dpy, scr));

		_win = win;
	}

	XftDrawStringUtf8(drw, color, font, x, y + font->ascent, (FcChar8 *)s,
			  strlen(s));
...
static struct pixmap *create_label_pixmap(struct hint *hints, size_t n,
					  const char *bgcol, const char *fgcol)
{
	...
	for (i = 0; i < n; i++) {
		...
		draw_text(pm->map, x + (boxw - sw) / 2, y, &xftcolor,
			  hint->label);
	}
...
static void draw_rounded_rectangle(Drawable dst, GC gc, unsigned int x,
				   unsigned int y, unsigned int w,
				   unsigned int h, unsigned int r)
{

	XFillArc(dpy, dst, gc, x, y, 2 * r, 2 * r, 64 * 90 * 1, 64 * 90);
	XFillArc(dpy, dst, gc, x + w - 2 * r, y, 2 * r, 2 * r, 64 * 90 * 0,
		 64 * 90);
	XFillArc(dpy, dst, gc, x + w - 2 * r, y + h - 2 * r, 2 * r, 2 * r,
		 64 * 90 * 3, 64 * 90);
	XFillArc(dpy, dst, gc, x, y + h - 2 * r, 2 * r, 2 * r, 64 * 90 * 2,
		 64 * 90);
...
static void apply_mask(Window win, uint8_t *indices)
{
	...
	for (i = 0; i < nhints; i++) {
		if (indices[i]) {
			...
			draw_rounded_rectangle(mask, gc, hint->x - boxw / 2,
					       hint->y - boxh / 2, boxw, boxh,
					       border_radius);
		}
	}
...
void init_hint(struct hint *_hints, size_t n, int height, int _border_radius,
	       const char *bgcol, const char *fgcol, const char *font_family)
{
	size_t i;
	int    sw, sh;
