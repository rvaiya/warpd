...
make && sudo make install && sudo launchctl load /Library/LaunchAgents/com.warpd.warpd.plist
```

*Note:* Some programs (e.g iTerm) have a 'secure input mode' that may need to be
disabled in order for warpd to work properly.

Changes to `~/.config/warpd/config` can be effected by reloading the system service using launchctl
E.G
